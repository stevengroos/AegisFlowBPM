...
export default function StoreProductDetail() {
  ...
  // Extraemos variantes o galería si vinieran en los datos crudos (raw_data)
  ...
  if (cargando) return <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col items-center justify-center"><Loader2 className="animate-spin w-12 h-12 mb-4" style={{color: storeInfo.themeColor}}/><p className="text-gray-500 font-bold uppercase tracking-widest">Cargando detalles...</p></div>;
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-3xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 md:p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
          
          {/* GALERÍA DE IMÁGENES */}
          <div className="flex flex-col gap-4">
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl p-8 border border-gray-100 dark:border-gray-800 flex items-center justify-center aspect-square md:aspect-[4/3] overflow-hidden">
              {imagenMostrar ? (
                 <img src={imagenMostrar} alt={producto.title} className="max-w-full max-h-full object-contain mix-blend-multiply dark:mix-blend-normal" />
              ) : (
            ...
            {gallery.length > 0 && (
              <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                <button onClick={() => { setImagenGaleriaSeleccionada(null); setVarianteSeleccionada(null); }} className={`w-20 h-20 rounded-xl bg-white border-2 overflow-hidden flex-shrink-0 p-2 transition-all ${(!varianteSeleccionada && !imagenGaleriaSeleccionada) ? 'border-gray-500' : 'border-gray-200'}`} style={(!varianteSeleccionada && !imagenGaleriaSeleccionada) ? {borderColor: storeInfo.themeColor} : {}}>
                  <img src={producto.image_url} alt="Portada" className="w-full h-full object-contain" />
                </button>
                {gallery.map((img, i) => (
                  <button key={i} onClick={() => { setImagenGaleriaSeleccionada(img.image_url); setVarianteSeleccionada(null); }} className={`w-20 h-20 rounded-xl bg-white border-2 overflow-hidden flex-shrink-0 p-2 transition-all ${imagenGaleriaSeleccionada === img.image_url ? 'border-gray-500' : 'border-gray-200'}`} style={imagenGaleriaSeleccionada === img.image_url ? {borderColor: storeInfo.themeColor} : {}}>
                    <img src={img.image_url} alt="Galeria" className="w-full h-full object-contain" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex flex-col">
            ...
            <div className="mb-6 flex flex-wrap gap-3">
              <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold ${stockMostrar > 0 ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50' : 'bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 border border-red-200 dark:border-red-800/50'}`}>
                {stockMostrar > 0 ? <><CheckCircle2 size={16}/> {stockMostrar} Disponibles</> : <><AlertTriangle size={16}/> Agotado temporalmente</>}
              </div>
            </div>

            <div className="mb-8 border-b border-gray-200 dark:border-gray-800 pb-6">
              <span className="text-sm font-bold text-gray-400 uppercase tracking-widest block mb-1">Precio Contado</span>
              <span className="text-4xl font-black" style={{color: storeInfo.themeColor}}>Gs. {precioBase.toLocaleString('es-PY')}</span>
            </div>

            {/* WIDGET DEL CUOTERO */}
            <div className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 p-6 rounded-3xl mb-8 relative overflow-hidden">
               ...
               <div className="flex bg-gray-200/50 dark:bg-gray-800 p-1 rounded-xl mb-6">
                  <button onClick={() => setModalidad('financiado')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${modalidad === 'financiado' ? 'bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500'}`}>Pagar en Cuotas</button>
                  <button onClick={() => setModalidad('contado')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${modalidad === 'contado' ? 'bg-white dark:bg-gray-700 shadow-sm text-gray-900 dark:text-white' : 'text-gray-500'}`}>Al Contado</button>
               </div>

               {modalidad === 'financiado' ? (
                 <div className="space-y-5 animate-in fade-in">
                    <div>
                      <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Plazo de Financiación</label>
                      <select value={cuotasElegidas} onChange={e => setCuotasElegidas(parseInt(e.target.value))} className="w-full p-3 bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-700 rounded-xl outline-none font-bold text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-blue-500">
                        {Object.keys(tasasExcel).map(mes => <option key={mes} value={mes}>{mes} Cuotas mensuales</option>)}
                      </select>
                    </div>
                    <div className="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800/50 rounded-xl p-5 text-center">
                       <span className="text-xs font-bold text-emerald-600 dark:text-emerald-500 uppercase tracking-widest block mb-1">Monto de la Cuota</span>
                       <span className="text-3xl font-black text-emerald-600 dark:text-emerald-400">Gs. {calcularCuotaIndividual().toLocaleString('es-PY')}</span>
                    </div>
                 </div>
               ) : (
                 <div className="animate-in fade-in text-center p-6">
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-widest block mb-2">Total a Pagar</span>
                    <span className="text-3xl font-black" style={{color: storeInfo.themeColor}}>Gs. {precioBase.toLocaleString('es-PY')}</span>
                 </div>
               )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-10">
               <button onClick={comprarPorWhatsApp} disabled={stockMostrar <= 0} className="flex-1 bg-[#25D366] hover:bg-[#20b858] text-white py-4 rounded-2xl font-black flex items-center justify-center gap-2 shadow-lg shadow-green-500/20 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                  <MessageCircle size={22}/> {stockMostrar > 0 ? 'Consultar Disponibilidad' : 'Agotado'}
               </button>
               <button onClick={agregarAlCarrito} disabled={stockMostrar <= 0} className="flex-1 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 border-2 py-4 rounded-2xl font-black flex items-center justify-center gap-2 shadow-sm transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed" style={stockMostrar > 0 ? {borderColor: storeInfo.themeColor, color: storeInfo.themeColor} : {borderColor: '#9ca3af', color: '#9ca3af'}}>
            ...
            {/* DESCRIPCIÓN */}
            <div>
               <h3 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-widest border-b border-gray-200 dark:border-gray-800 pb-3 mb-4 flex items-center gap-2">
                 <ChevronRight size={18} style={{color: storeInfo.themeColor}}/> Detalles del Producto
               </h3>
               <div className="prose dark:prose-invert prose-sm max-w-none text-gray-600 dark:text-gray-400 leading-relaxed" dangerouslySetInnerHTML={{ __html: formatearDescripcion(producto.description) }} />
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              {carrito.length === 0 ? (
                ...
                <div className="space-y-4">
                  {carrito.map(item => (
                    <div key={item.idUnicoInterno} className="flex gap-4 p-4 bg-white dark:bg-gray-950 border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm">
                      ...
                      <div className="flex-1 flex flex-col">
                        ...
                        {item.color_name && <span className="text-xs text-gray-500 mb-1">Opción: {item.color_name}</span>}
                        
                        {item.modalidadElegida === 'financiado' ? (
                          <span className="text-emerald-500 font-bold text-xs">{item.cuotasElegidas} x Gs. {item.valorCuotaCalculado?.toLocaleString('es-PY')}</span>
                        ) : (
                        ...
                        <div className="flex justify-between items-center mt-auto pt-3">
                          <div className="flex items-center gap-3 bg-gray-100 dark:bg-gray-800 rounded-lg px-2 py-1">
                            <button onClick={() => modificarCantidad(item.idUnicoInterno, -1)} className="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-black dark:hover:text-white font-bold">-</button>
                            <span className="text-xs font-bold w-4 text-center">{item.cantidad}</span>
                            <button onClick={() => modificarCantidad(item.idUnicoInterno, 1)} className="w-6 h-6 flex items-center justify-center text-gray-500 hover:text-black dark:hover:text-white font-bold">+</button>
                          </div>
                          <button onClick={() => eliminarDelCarrito(item.idUnicoInterno)} className="text-[11px] font-bold text-red-500 hover:underline">Quitar</button>
                        </div>
                      </div>
                    </div>
                  ))}
