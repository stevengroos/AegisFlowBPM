...
import { FileText, Trash2, Loader2, Plus, Sparkles } from 'lucide-react'; 
...
const FileUploadField = ({ type, value, onChange, disabled, expectedFields, onDataExtracted }) => {
  ...
  if (value) {
    // 🔥 PENTEST FIX: Resolvemos si el enlace es local (Backend) o absoluto (Supabase/S3)
    const fileUrl = value.startsWith('http') ? value : `${baseURL}${value}`;
      ...
      <div className="flex flex-col gap-2 w-full">
          <div className="relative border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 rounded-xl p-3 flex items-center justify-between group transition-colors w-full">
            <div className="flex items-center gap-3 overflow-hidden">
              {type === 'image' ? (
                // 🔥 AQUÍ APLICAMOS LA VARIABLE fileUrl
                <img src={fileUrl} alt="Preview" className="h-10 w-10 object-cover rounded-lg shadow-sm border border-gray-200 dark:border-gray-700" />
              ) : (
              ...
              {/* 🔥 Y AQUÍ TAMBIÉN PARA LA DESCARGA */}
              <a href={fileUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-bold text-blue-600 dark:text-blue-400 hover:underline truncate">
                {value.split('/').pop()}
              </a>
            </div>
            {!disabled && (
